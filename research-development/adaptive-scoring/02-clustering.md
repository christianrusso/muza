...
> **En resumen**: para calibrar el score, los outfits se agrupan entre sí por parecido (mismo tipo de prenda, color, estilo) — nunca por datos de la persona (edad, ubicación), para no meter sesgo. El agrupado corre solo, sin librerías nuevas, y se puede medir automáticamente si un grupo quedó bien armado (sin que nadie lo revise a mano). Límite conocido: esto no captura del todo diferencias culturales o geográficas (ej. Buenos Aires vs. Beijing).
...
- **Almacenamiento**: Postgres/Supabase (`outfit_clusters` o `cluster_id` en `analyses`, ver [00-phase-1-feedback-clustering-fewshot.md](./phases/00-phase-1-feedback-clustering-fewshot.md#12-clustering-básico-de-outfits-2-3-semanas)). Se recalcula por lotes (una vez al día, junto al resto de los procesos de esta iniciativa), no al instante — más simple, y no hay ninguna parte de la app que necesite el cluster de un análisis apenas se crea.
