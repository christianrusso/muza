...
-- Compatibilidad con feat-1: si `post_comments.hidden_at` ya existe, conserva
-- la regla de que los comentarios moderados no participan en el contador.
do $block$
begin
  if exists (
    select 1
    ...
    execute $view$
      ...
      select
        ...
        coalesce((select count(*) from public.post_reactions r where r.post_id = p.id and r.reaction_type = 'like' and not public.users_are_blocked(auth.uid(), r.user_id)), 0)::integer as likes_count,
        coalesce((select count(*) from public.post_reactions r where r.post_id = p.id and r.reaction_type = 'dislike' and not public.users_are_blocked(auth.uid(), r.user_id)), 0)::integer as dislikes_count,
        coalesce((select count(*) from public.post_comments c where c.post_id = p.id and c.hidden_at is null and not public.users_are_blocked(auth.uid(), c.author_id)), 0)::integer as comments_count,
        coalesce((select count(*) from public.post_votes v where v.post_id = p.id and not public.users_are_blocked(auth.uid(), v.user_id)), 0)::integer as votes_count
      from public.community_posts p
      join public.profiles pr on pr.id = p.author_id
      where not public.users_are_blocked(auth.uid(), p.author_id)
    $view$;
  end if;
end
$block$;
...
begin
  if exists (
    ...
    execute 'drop policy if exists "post_comments_select_authenticated" on public.post_comments';
    execute 'create policy "post_comments_select_authenticated" on public.post_comments for select to authenticated using (hidden_at is null and not public.users_are_blocked(auth.uid(), user_id))';
  end if;
end
$block$;
  ...
  select (
    ...
    (select count(*)
     ...
     where f.following_id = (select id from me)
       ...
       and not public.users_are_blocked(auth.uid(), f.follower_id))
  )::int;
$$;
