...
- `user_block_history`: ciclos de bloqueo social con fecha de bloqueo y desbloqueo;
  se conserva para trazabilidad y métricas, con RLS para ocultar relaciones privadas.
...
- `0023`: historial de bloqueos sociales, RPCs/RLS, filtros de comunidad y métricas admin.
...
La migración `0023_user_blocks.sql` agrega `user_block_history`. El bloqueo social
es distinto de `profiles.blocked_at`: elimina follows en ambas direcciones, filtra
contenido e interacciones entre las personas y no borra datos históricos.
...
- Las consultas comunitarias deben aplicar el filtro bidireccional de bloqueo social;
  las métricas admin se calculan desde datos persistidos y no desde PostHog.
